...
def score_pair(
        ...
        if embeddings_q and embeddings_c:
            ...
            if len(row_ind) > 0:
                semantic_union = float(
                    np.mean([sem_matrix[i, j] for i, j in zip(row_ind, col_ind)])
                )
            else:
                semantic_union = 0.0
        else:
    ...
    is_join_gt = (
        1
        if (join_score >= JOIN_THRESHOLD and semantic_join >= SEMANTIC_THRESHOLD)
        else 0
    )
    is_union_gt = (
        1
        if (union_score >= UNION_THRESHOLD and semantic_union >= SEMANTIC_THRESHOLD)
        else 0
    )
...
def main():
    ...
    max_candidates_per_query = (
        None  # None = all candidates, or set to number (e.g., 50)
    )
    ...
    for q_idx, q_file in enumerate(query_files, 1):
        ...
        for c_file in candidates:
            ...
            (
                ...
                is_union_gt,
                is_gt,
            ) = score_pair(q_name, df_q, profile_q, c_name, df_c, profile_c)
